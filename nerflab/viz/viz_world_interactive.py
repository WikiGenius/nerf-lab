...
# -----------------------------------------------------------------------------
# Utilities (NumPy/Torch adapters, mesh builders, camera/frustum glyphs)
# -----------------------------------------------------------------------------
...
def _as_long_tensor(
    ...
    """Accept None, (K,), or (B,K) and return torch.long on `device`."""
    if x is None:
        return None
...
def _camera_frustum_lines(C: np.ndarray, R: np.ndarray, *, fov_deg: float = 60.0, scale: float = 0.2):
    """Return a list of (2,3) line segments representing a simple frustum."""
    f = float(scale)
    ...
    lines = [np.stack([C, q], axis=0) for q in corners_w]
    loop = [0, 1, 2, 3, 0]
    for i in range(4):
        a = corners_w[loop[i]]
...
def _add_lines3d(fig: go.Figure, lines: Sequence[np.ndarray], name: str, *, width: float = 2.0, opacity: float = 1.0):
    for seg in lines:
        seg = _to_numpy3(seg)
...
# ----- Box helpers (match your Box dataclass: center + size, bounds property) -
...
def _triangulated_box_vertices_faces(box: Box) -> tuple[np.ndarray, np.ndarray]:
    ...
    if hasattr(box, "corners_world"):
        ...
        # Fallback to center/size names used in your dataclass (NOT 'dims')
...
# ----- Sphere mesh (lat-long grid → triangles) --------------------------------
...
def _grid_to_tris(nv: int, nu: int):
    tri_i, tri_j, tri_k = [], [], []
    def idx(a, b): return a * nu + b
    for a in range(nv - 1):
        for b in range(nu - 1):
            p00 = idx(a, b); p01 = idx(a, b + 1)
            p10 = idx(a + 1, b); p11 = idx(a + 1, b + 1)
...
def plot_world_interactive(
    ...
    *,
    ...
    # style / opacity (mirrors Matplotlib defaults via VCFG)
    shape_face_alpha: float | None = None,
    shape_edge_alpha: float | None = None,   # we draw edges as lines
    ray_alpha: float | None = None,
    ray_linewidth: float | None = None,
    samples_alpha: float | None = None,
    # rays / samples
    draw_rays: bool = False,
    ray_mode: Literal["lines", "quiver"] = "lines",  # 'quiver' approximated via short segments
    ...
    Mirrors the Matplotlib `plot_world(...)` semantics, including:
      • single or batched `Camera`
      • `cam_indices` selection
      • rays from step grid, uniform subsample, or explicit `rays_indices`
      • optional sampling along rays (or plot external points)
      • near/far control over visible ray length

    Returns
    -------
    (fig, None) : Use as `(fig, ax)` compat in call sites.
    """
    ...
    for cam in cams:
            ...
            if is_batched:
                if idx_user.ndim == 1:
                    ...
                    if idx_user.shape[0] == cam.B:
                        ...
                        # lift (Bsel,K) → (B,K) where non-selected rows copy row0
        ...
        for k in range(len(sel)):
            ...
            fr_lines = _camera_frustum_lines(C, R, fov_deg=60.0, scale=0.2 * seg_len_for_quiver)
            _add_lines3d(fig, fr_lines, name=f"cam{sel[k] if is_batched else 0}", width=2.0, opacity=1.0)
