# nerflab/camera.py (clean & deduplicated)
...
class Camera:
    ...
    If constructor parameters are omitted, values fall back to global CFG.
    ...
    def get_rays(
        ...
        *,
        ...
        # Build pixel grid; defer composition to a single implementation
        ...
        if step > H and step > W:
            raise ValueError(f"step={step} is too large for resolution H={H}, W={W} (no rays).")
        if u.numel() == 0:  # (v matches; one check is enough)
            H, W = int(self.intr.height), int(self.intr.width)
            raise ValueError(
                f"step={step} is too large for resolution H={H}, W={W} (no rays)."
            )

        O, D = self._compose_rays_from_uv(
            u=u, v=v, frame=frame, normalize=normalize, device=device, dtype=dtype
        )
        ...
        *,
        ...
        if indices is not None:
            if self._is_batched:
                ...
                idx = indices.to(device=device, dtype=torch.long)  # (B,K)
                v_idx = (idx // Ww) * step  # (B,K)
                u_idx = (idx %  Ww) * step  # (B,K)
            else:
                if indices.ndim != 1:
                    raise ValueError("indices must be (K,) for unbatched sampling")
                idx = indices.to(device=device, dtype=torch.long)  # (K,)
                v_idx = (idx // Ww) * step  # (K,)
                u_idx = (idx %  Ww) * step  # (K,)
            return self._compose_rays_from_uv(
                u=u_idx, v=v_idx, frame=frame, normalize=normalize,
    ...
    def set_poses(
        ...
        self._rays_cache.clear()  # pixel-grid cache still valid if (W,H,step,device,dtype) unchanged
    ...
    def _compose_rays_from_uv(
        ...
        *,
        ...
        """
        Build (O,D) for given (u,v) without building the full grid.
        • Unbatched input: u,v -> (K,)
        • Batched input  : u,v -> (K,) (broadcast across B)  OR (B,K)
        Returns (O,D): (K,3) or (B,K,3)
        """
        ...
        if not self._is_batched:
            ...
            # Clone so different cache keys don't alias the same storage
            O = t_wc.expand_as(D).clone()
            return O.to(dtype=dtype), D.to(dtype=dtype)

        # Batched
        B = self.B
        if u.ndim == 1:
            # Broadcast same u,v to all poses
            K = int(u.shape[0])
            dirs_c = self._dirs_cam(u.to(dtype=dtype), v.to(dtype=dtype), normalize=need_norm_cam)  # (K,3)
            if frame == "camera":
            ...
            if normalize:
                D = D / D.norm(dim=-1, keepdim=True).clamp_min(1e-12)
            O = t_wc[:, None, :].expand(B, K, 3).clone()
            return O.to(dtype=dtype), D.to(dtype=dtype)
